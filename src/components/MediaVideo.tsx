...
export default function MediaVideo({url,title,image}:{url:string;title:string;image:string}){
 ...
 if(!url)return <div className="media-pending">{image&&<img src={image} alt="" width="480" height="270" loading="lazy" />}<p>{title} — vidéo à venir</p></div>;
 if(!playing)return <button className="media-play" type="button" onClick={()=>setPlaying(true)} aria-label={"Lire : "+title}>{image&&<img src={image} alt="" width="480" height="270" loading="lazy" />}<span>▶ Lire la vidéo</span></button>;
