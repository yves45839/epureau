...
export default function ContentField({field,value,onChange}:{field:Field;value:string;onChange:(value:string)=>void}){
 ...
 return <div className="admin-field"><label>{field.label}{field.type==="long"?<textarea rows={4} value={value} onChange={e=>onChange(e.target.value)} maxLength={24000} />:field.type==="mediaType"?<select value={value} onChange={e=>onChange(e.target.value)}><option value="photo">Photo</option><option value="video">Vidéo</option></select>:field.type==="documentType"?<select value={value||"Fiche technique"} onChange={e=>onChange(e.target.value)}><option>Fiche technique</option><option>Brochure fabricant</option></select>:field.type==="brand"?<select value={value||marquesProduits[0]} onChange={e=>onChange(e.target.value)}>{marquesProduits.map(m=><option key={m} value={m}>{m}</option>)}</select>:field.key==="blogEnabled"?<select value={value} onChange={e=>onChange(e.target.value)}><option value="non">Non</option><option value="oui">Oui</option></select>:<input value={value} onChange={e=>onChange(e.target.value)} maxLength={24000} />}</label>{["image","url"].includes(field.type||"")&&<><label className="admin-upload">{busy?"Téléversement…":"Choisir un fichier · 4 Mo max"}<input type="file" accept={field.key==="fiche"?"application/pdf":field.type==="image"?"image/jpeg,image/png,image/webp":"image/jpeg,image/png,image/webp,application/pdf"} disabled={busy} onChange={e=>upload(e.target.files?.[0])} /></label>{field.type==="image"&&value&&<img className="admin-image-preview" src={value} alt="Aperçu du visuel" />}</>}{error&&<p role="alert" className="admin-error">{error}</p>}</div>;
}
