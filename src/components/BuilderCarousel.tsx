...
export default function BuilderCarousel({block}:{block:BuilderBlock}){
 ...
 return <div className="builder-carousel" ref={root} role="region" aria-roledescription={ui("carrousel")} aria-label={block.title||ui("Galerie")} onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)} onFocusCapture={e=>setFocused(!e.target.closest(".builder-carousel-controls"))} onBlurCapture={e=>{if(!e.currentTarget.contains(e.relatedTarget))setFocused(false);}}>
  {block.items.map((item,i)=><figure key={item.id} hidden={i!==active} role="group" aria-roledescription={ui("diapositive")} aria-label={`${i+1} ${ui("sur")} ${block.items.length}`}>
   {item.image&&<img src={item.image} alt={item.alt} width="1200" height="650" loading="lazy" />}<figcaption><h3>{item.title}</h3><p>{item.text}</p>{item.href&&<a href={item.href}>{ui("En savoir plus →")}</a>}</figcaption>
  </figure>)}
  {block.items.length>1&&<div className="builder-carousel-controls"><button type="button" aria-label={ui("Diapositive précédente")} onClick={()=>move(-1)}>←</button><span aria-live={playing?"off":"polite"}>{active+1} / {block.items.length}</span><button type="button" onClick={()=>setPlaying(v=>!v)}>{ui(playing?"Pause":"Lecture automatique")}</button><button type="button" aria-label={ui("Diapositive suivante")} onClick={()=>move(1)}>→</button></div>}
 </div>;
}
