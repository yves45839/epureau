...
export type Account = { email: string; name: string; role: Role; password: string; active: boolean };
export function may(role: Role, section: string) {
  if (role === "admin") return true;
  if (section === "dashboard") return true;
  return role === "editeur" ? ["pages","projects","media","brochures","blog"].includes(section) : section === "requests";
}
