...
export default async function Blog(){
 ...
 return <><PageHeader fil={["Blog"]} titre="Actualités et expertise" lead="Les nouvelles et les conseils d’EPUREAU Côte d’Ivoire" /><section className="sec"><div className="wrap career-domains">{articles.map(({key,data})=><Link key={key} href={"/blog/"+key}>{data.image&&<img src={data.image} alt="" width="640" height="360" loading="lazy" />}<h2>{data.title}</h2><p>{data.description}</p></Link>)}{!articles.length&&<p>Aucun article publié pour le moment.</p>}</div></section></>;
}
